...
# 创建JsonOutputKeyToolsParser实例，用于解析JSON输出中的特定键值
# 该解析器将查找名为get_weather的工具输出，并仅返回第一个匹配的工具结果
# first_tool_only参数设置为True，表示仅返回第一个匹配的工具结果
...
# 执行完整链路，查询上海天气并打印结果
invoke = full_chain.invoke("请问济南今天天气如何？")
logger.info(invoke)
